...
export default function Page({data: pageListData}){
    const {fileBase} = usePage().props
    const [pagination, setPagination] = useState({page: pageListData.page, length: pageListData.length})
    ...
    useEffect(() => {
        setPaginationFromSessionStorage(setPagination, 'page_pagination', {
            page: pageListData?.page, length: pageListData?.length, status: pageListData.status, searchText: ""
        }, )
    }, []);
    ...
    useEffect(() => {
        if (pagination?.page && pagination?.length && go) {
            ...
            router.get(route('admin.page.list', queryParams))

            sessionStorage.setItem('page_pagination', JSON.stringify(pagination))
        }
    }, [go]);
                    ...
                    <div className="w-full p-5">
                        <div className="flex flex-col md:flex-row md:justify-between  items-center gap-3 my-4 text-[.75rem]">
                            ...
                            <div className="flex flex-col md:flex-row items-center md:justify-end gap-3 flex-wrap">
                                <StatusFilter status={pageListData.pageStatus} pagination={pagination} setPagination={setPagination} setGo={setGo} segment="Product" />
                        ...
                        <div className="overflow-x-auto">
                                ...
                                <tbody>
                                {
                                    pageListData.pages.map((page) => (
                                        <tr key={page.id} className={`hover:bg-gray-100 dark:hover:bg-neutral-600 ${page.status === STATUS_DELETED ? 'bg-red-300 hover:bg-red-300 dark:hover:bg-red-300' : ''}`}>
                                            ...
                                            <td className="px-4 py-2 font-medium text-gray-800 dark:text-neutral-200 border border-neutral-300 dark:border-neutral-400">
                                                {
                                                    page.status === STATUS_DELETED ? (<span className={`capitalize`}>{page.status}</span>) : (
                                                        <select
                                                            ...
                                                            value={page.status}
                                                            onChange={(e) => {
                                                                router.post(route('admin.page.change_status'), {
                                                                    id: page.id, status: e.target.value
                                                                })
                                                            }}
                                                        >
                                                            {Object.keys(pageListData.pageStatus).filter(key => key !== STATUS_DELETED).map((key) => {
                                                                const statusText = pageListData.pageStatus[key];
                            ...
                            <div className="py-3 sm:px-6">
                                <div className="flex flex-col sm:flex-row gap-2 items-center justify-between">
                                    <Summary
                                        page={pageListData.page}
                                        length={pageListData.length}

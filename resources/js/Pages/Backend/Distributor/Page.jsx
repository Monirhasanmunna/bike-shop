...
import DistributorForm from "@/Components/Segment/Backend/Distributor/DistributorFrom.jsx";

export default function Page({data: distributorListData}){
    const {fileBase} = usePage().props
    const [pagination, setPagination] = useState({page: distributorListData.page, length: distributorListData.length})
    ...
    useEffect(() => {
        setPaginationFromSessionStorage(setPagination, 'distributor_pagination', {
            page: distributorListData?.page, length: distributorListData?.length, status: distributorListData.status, searchText: ""
        }, )
    }, []);
    ...
    useEffect(() => {
        if (pagination?.page && pagination?.length && go) {
            ...
            router.get(route('admin.distributor.list', queryParams))

            sessionStorage.setItem('distributor_pagination', JSON.stringify(pagination))
        }
    }, [go]);


    const editDistributor = (distributor, imagePath=null) => {
        setDistributor({
            ...distributor,
            imagePath
        })
        HSOverlay.open('#distributor-form')
    }
    ...
    const alert = (id) => {
        setAlertData({
            ...alertData,
            route : route('admin.distributor.delete', id),
            color : 'red',
            method : 'delete',
    ...
    return (
        <>
            <Main>
                ...
                <div className="w-full border border-gray-300 rounded mt-5">
                    <div className="flex items-center justify-between border-b border-gray-300 px-5 py-3">
                        ...
                        <button onClick={() => setDistributor(null)} type="button"
                                ...
                                aria-haspopup="dialog" aria-expanded="false" aria-controls="distributor-form"
                                data-hs-overlay="#distributor-form">
                            Add New
                        </button>
                    </div>
                    ...
                    <div className="w-full p-5">
                        <div className="flex flex-col md:flex-row md:justify-between  items-center gap-3 my-4 text-[.75rem]">
                            ...
                            <div className="flex flex-col md:flex-row items-center md:justify-end gap-3 flex-wrap">
                                <StatusFilter status={distributorListData.distributorStatus} pagination={pagination} setPagination={setPagination} setGo={setGo} segment="Distributor" />
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full border border-neutral-300 dark:border-neutral-400 border-collapse text-sm">
                                ...
                                <tbody>
                                {
                                    distributorListData.distributors.map((distributor) => (
                                        <tr key={distributor.id} className={`hover:bg-gray-100 dark:hover:bg-neutral-600 ${distributor.status === STATUS_DELETED ? 'bg-red-300 hover:bg-red-300 dark:hover:bg-red-300' : ''}`}>
                                            <td className="px-4 py-2 font-medium text-gray-800 dark:text-neutral-200 border border-neutral-300 dark:border-neutral-400">
                                                <div className="w-[90px] h-[50px] border border-gray-300">
                                                    <img src={fileBase + '/' + distributor.image} className={`w-full h-full object-cover`} alt="distributor"/>
                                                </div>
                                            </td>
                                            <td className="px-4 py-2 font-medium text-gray-800 dark:text-neutral-200 border border-neutral-300 dark:border-neutral-400">
                                                <span dangerouslySetInnerHTML={{ __html: distributor.name }} />
                                            </td>
                                            <td className="px-4 py-2 font-medium text-gray-800 dark:text-neutral-200 border border-neutral-300 dark:border-neutral-400">
                                                {
                                                    distributor.status === STATUS_DELETED ? (<span className={`capitalize`}>{distributor.status}</span>) : (
                                                        <select
                                                            ...
                                                            onChange={(e) => {
                                                                router.post(route('admin.distributor.change_status'), {
                                                                    id: distributor.id, status: e.target.value
                                                                })
                                                            }}
                                                        >
                                                            {Object.keys(distributorListData.distributorStatus).filter(key => key !== STATUS_DELETED).map((key) => {
                                                                const statusText = distributorListData.distributorStatus[key];
                                            ...
                                            <td className="px-4 py-2 font-medium text-gray-800 dark:text-neutral-200 border border-neutral-300 dark:border-neutral-400">
                                                <div className="w-full flex gap-x-6">
                                                    <button className="cursor-pointer text-white border-green-500"
                                                            onClick={() => editDistributor(distributor, fileBase + '/' + distributor.image)}>
                                                        <FaPencil className={`text-blue-400 text-lg`}/>
                                                    </button>
                                                    <button className=" cursor-pointer text-white border-green-500"
                                                            onClick={() => alert(distributor.id)}>
                            ...
                            <div className="py-3 sm:px-6">
                                <div className="flex flex-col sm:flex-row gap-2 items-center justify-between">
                                    <Summary
                                        page={distributorListData.page}
                                        length={distributorListData.length}
                                        count={distributorListData.count}
                                    />
                                    <Navigation
                                        page={pagination.page}
                                        length={distributorListData.length}
            ...
            <DistributorForm distributor={distributor} setDistributor={setDistributor} />
